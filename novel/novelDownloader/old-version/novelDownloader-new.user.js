...
var App = {
  ...
  notification(text, call) { // 桌面通知
    ...
    if (CONFIG.isChrome) {
      ...
      Notification.requestPermission((status) => {
        if (status === 'granted') {
          const n = new Notification(title || body, title ? { body } : null);
          setTimeout(() => {
            if (n) n.close();
          }, 1000 * 5);
          n.onclick = function () {
  ...
  getChapters() { // 获取章节
    const rule = _('rule');
    const chapter = _('chapter');
    const inWait = _('inWait');
    let order = 0;
    const out = [];
    let a; let
      b;
    const range = $('#nd>div input[name=range]').val().split(',');
    range.forEach((i) => {
      if (/^\d+$/.exec(i)) {
        out.push(i * 1);
      } else {
  ...
  saveAs(type) { // 保存为...
    ...
    const rule = _('rule');
    const chapter = _('chapter');
    const leng = String(chapter.length).length; // 补足数字用
    let title; // rule.title
    title = rule.title instanceof RegExp ? document.title.match(rule.title)[1] : $(rule.title).text();
    const href = `来源地址: ${location.href}`;
    const writer = `作者: ${'writer' in rule ? $(rule.writer)[0].textContent.trim() : '未知'}`;
    const lastUpdate = `上次更新时间: ${'lastUpdate' in rule ? $(rule.lastUpdate)[0].textContent.trim() : '未知'}`;
    const now = `小说制作时间: ${new Date().toLocaleString()}`;
    const intro = `小说简介: \r\n${'intro' in rule ? App.format($(rule.intro).html()) : '未知'}`;
    if (type === 'text') {
      let _text;
      _text = `${title}\r\n`;
      _text = `${writer}\r\n`;
      _text = `${_text}${CONFIG.note}\r\n`;
      _text = `${_text}${lastUpdate}\r\n`;
      _text = `${_text}${now}\r\n`;
      _text = `${_text}${intro}\r\n`;
      _text = `${_text}\r\n`;
      chapter.forEach((i) => {
      ...
      const lang = CONFIG.lang === 0 ? 'zh' : CONFIG.lang === 1 ? 'zh-CN' : 'zh-TW';
      blob = new JSZip();
      ...
      const OEBPS = blob.folder('OEBPS');
      const html = OEBPS.folder('html');
      let image;
      ...
      for (let i = 0; i < chapter.length; i++) {
        ...
        if (CONFIG.image && chapter[i].image) {
          ...
          const leng = String(_('imageOrder')).length;
          let name;
          _text = $(`<div>${_text}</div>`);
          for (const j in chapter[i].image) {
        ...
        toc_ncx = `${toc_ncx}<navPoint id="chapter${order}" playOrder="${i + 1}"><navLabel><text>${_title}</text></navLabel><content src="html/${order}.html"/></navPoint>`;
        item = `${item}<item id="chapter${order}" href="html/${order}.html" media-type="application/xhtml+xml"/>`;
        itemref = `${itemref}<itemref idref="chapter${order}" linear="yes"/>`;
  ...
  saveThis() { // 保存本页
    let blob;
    const rule = _('rule');
    const title = $(rule.chapterTitle).text().trim();
  ...
  xhr(i) { // 下载文本
    ...
    inProgress[i] = GM_xmlhttpRequest({
      ...
      onload(response) {
        const chapter = _('chapter');
        const inProgress = _('inProgress');
        const content = $(_('rule').content, response.responseText);
        ...
        _('chapter', chapter);
        _('inProgress', inProgress);
        const value = $('.ndProgress>progress').val() + 1;
  ...
  format(text) { // 文本格式化
    ...
    if (_('CONFIG').type === 'epub') {
      ...
      if ('contentRemove' in _('rule')) {
        _('rule').contentRemove.forEach((i) => {
          text = text.replace(new RegExp(i, 'gi'), '');
        });
      }
      if ('contentReplace' in _('rule')) {
        for (const i in _('rule').contentReplace) {
  ...
  imageTask(url) { // 图片任务列队
    const imageTask = _('imageTask');
