...
(function() {
  ...
  if ($('#lst-ib').val().match(/^[a-zA-Z\s-]+$/)) {
    var appid = '20170910000081816';
    var q = $('#lst-ib').val();
    var salt = new Date().getTime();
    var pass = 'YGwmW30xFQZxDiOxcANS';
    //
    GM_xmlhttpRequest({
      method: 'POST',
      ...
      onload: function(res) {
        if (res.status === 200) {
          ...
          if ('error_code' in data) {
            ...
            info = '百度Api翻译结果: <br>';
            data.trans_result.forEach(function(i) {
              info += '<a href="https://translate.google.com.hk/#en/zh-CN/' + encodeURIComponent(i.src) + '" target="_blank">' + i.src + '</a>' + ': ' + i.dst;
            });
          }
  ...
  $('<a class="fl" target="_blank">网页快照</a>').appendTo('.f.kv._SWb').attr('href', function() {
    return '/search?q=cache%3A' + encodeURIComponent($(this).parentsUntil('.rc').eq(-1).prev().find('a').prop('href'));
  });
  //添加标签
  $('<a class="fl">Tag</a>').on({
    click: function() {
      editTags($(this).parentsUntil('.rc').eq(-1).prev().find('a').prop('host'));
      updateTags($(this).parentsUntil('.rc').eq(-1).prev().find('a')[0]);
    }
  }).appendTo('.f.kv._SWb');
  //添加到黑名单
  $('<a class="fl">Blacklist</a>').on({
    click: function() {
      editTags($(this).parentsUntil('.rc').eq(-1).prev().find('a').prop('host'), 'block');
      updateTags($(this).parentsUntil('.rc').eq(-1).prev().find('a')[0]);
    }
  }).appendTo('.f.kv._SWb');
  //显示标签与隐藏黑名单
  ...
  function updateTags(obj) {
    ...
    if (host in tags) {
      ...
      $(obj).parentsUntil('.srg').eq(-1).find('.f.kv._SWb>.tags').remove();
      $('<div class="tags"></div>').html(function() {
        html = [];
        _tag.forEach(function(i) {
