...
(function () {
  ...
  if (/\/g\/\d+\//.test(document.URL)) { //信息页
    var name = jQuery('h1#gn').text();
    var gid = document.URL.split('/') [4];
    var token = document.URL.split('/') [5];
    var _info = {
      'name': name,
      'gid': gid,
      'token': token
    ...
    document.onkeydown = function (e) {
      if (e.shiftKey && ((e.keyCode >= 49 && e.keyCode <= 57) || e.keyCode === 65)) { //Shift+Num/Shift+A
        ...
        var status = (e.keyCode === 65) ? 10 : e.keyCode - 48;
        _info.status = status;
        whitelist[gid] = _info;
        GM_setValue('whitelist', whitelist);
        jQuery('#gd3>div>div').attr('class', function () {
          return (status === 10) ? 'ehLBMark' : 'i' + gid + ' ehLBFav ehLBFav' + status
        });
        jQuery('#gdf').html(function () {
          return (status === 10) ? '<div class="ehLBMark"></div>已加入[#作者]' : '<div class="ehLBFav ehLBFav' + status + '"></div>' + intro[status];
        });
    ...
    jQuery('.it5>a').each(function () {
      ...
      var name = this.innerText.replace(/\[.*?\]|\(.*?\)/g, '').replace(/^\s+|\s+$/g, '').replace(/\|.*/, '');
    ...
    jQuery('.ehLBMark,.ehLBFav,.ehLBAdd,.ehLBUnlike').mousedown(function (e) {
      ...
      var whitelist = GM_getValue('whitelist', new Object());
      var blacklist = GM_getValue('blacklist', new Array());
      var name = parent.find('.it5>a').text();
      var gid = parent.find('.it5>a').attr('href').split('/') [4];
      var token = parent.find('.it5>a').attr('href').split('/') [5];
      var _info = {
        'name': name,
        'gid': gid,
        'token': token
  ...
  GM_registerMenuCommand('显示黑名单', function () {
    if (jQuery('.ehLBBox').length === 0) {
      var blacklist = GM_getValue('blacklist', new Array());
      var _html = '<div class="ehLBBox"><div class="ehLBTable"><table><tbody>';
      for (var i = 0; i < blacklist.length; i++) {
        _html += '<tr><td><a href="/?f_search=%22' + encodeURIComponent(blacklist[i]) + '" target="_blank">' + blacklist[i] + '</a></td><td><input type="checkbox" name="' + blacklist[i] + '"></input></td></tr>';
      }
      _html += '</tbody></table></div><button class="ehLBHideBox">X</button><button class="ehLBDedup">去重</button><button class="ehLBDelete">删除</button></div>';
      jQuery('body').append(_html);
      jQuery('.ehLBHideBox').click(function () {
        jQuery('.ehLBBox').remove();
      });
      jQuery('.ehLBDedup').click(function () {
        blacklist.sort();
  ...
  GM_registerMenuCommand('显示收藏夹', function () {
    if (jQuery('.ehLBBox').length === 0) {
      var whitelist = GM_getValue('whitelist', new Object());
      var _html = '<div class="ehLBBox"><div class="ehLBTable"><table><tbody><th>名字</th><th></th>';
      ...
      _html += _htmlArr.join('') + '</tbody></table></div><button class="ehLBHideBox">X</button><button class="ehLBDelete">删除</button></div>';
      jQuery('body').append(_html);
      jQuery('.ehLBHideBox').click(function () {
        jQuery('.ehLBBox').remove();
      });
