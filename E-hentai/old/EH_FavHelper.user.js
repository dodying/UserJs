...
(function () {
  if (!document.querySelector('#taglist')) { // 搜索页
    ...
    const tr = document.querySelectorAll('.itg>tbody>tr');
    if (!document.querySelector('.itg>tbody>tr>td>input')) {
      ...
      tr.forEach((_tr, i) => {
        const div = (i === 0) ? document.createElement('th') : document.createElement('td');
        div.style = 'text-align:center;';
        div.innerHTML = `<input id="EH_FavHelper_${i}" type="checkbox">`;
        if (i === 0) {
          div.querySelector('input').addEventListener('click', function () { // 全选
            const _this = this;
            document.querySelectorAll('tr.gtr0 input,tr.gtr1 input').forEach((i) => {
              i.checked = (_this.checked === true);
            });
          });
        }
        div.querySelector('input').addEventListener('change', function () { // 高亮选中
          const a = this.parentNode.parentNode;
    ...
    Open.onmousedown = function (event) {
      const input_check = document.querySelectorAll('.itg>tbody>tr>td>input:checked');
      // console.log(input_check);
      input_check.forEach((i) => {
        const _tr = i.parentNode.parentNode;
    ...
    Batch.onclick = function () {
      ...
      document.querySelectorAll('tr.gtr0,tr.gtr1').forEach((i) => {
