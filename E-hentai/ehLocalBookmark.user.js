...
(function () {
  ...
  if (/\/g\/\d+\//.test(document.URL)) { //信息页
    ...
    var time = jQuery('td:contains(Posted)').next().text();
    var thumb = jQuery('#gd1>img').attr('src');
    var _info = {
      'name': name,
      'gid': gid,
      'token': token,
    ...
    document.onkeydown = function (e) {
      if (e.shiftKey && ((e.keyCode >= 49 && e.keyCode <= 57) || e.keyCode === 65)) { //Shift+Num
        var whitelist = GM_getValue('whitelist', new Object());
        var status = (e.keyCode === 65) ? 0 : e.keyCode - 48;
        _info.status = status;
        whitelist[gid] = _info;
    ...
    jQuery('.ehLBFav,.ehLBAdd,.ehLBUnlike').mousedown(function (e) {
      ...
      var thumb = parent.find('.it2>img').attr('src');
      var _info = {
        'name': name,
        'gid': gid,
        'token': token,
    ...
    if (jQuery('.ehLBBox').length === 0) {
      ...
      jQuery('.ehLBName>a').on({
        mousemove: function (e) {
          jQuery('.ehLBThumb').show();
          jQuery('.ehLBThumb').css({
            'left': function () {
...
function addStyle() {
  ...
  + '.ehLBFav,.ehLBAdd,.ehLBUnlike,.ehLBSearch{cursor:pointer;margin:0 1px;float:left;width:16px;height:16px;}'
