...
var EH_QuickSearch_Button_innerHTML = '<a href="javascript:#" onmousedown="switch (event.button){case 0:var div = document.querySelector(\'#EH_QuickSearch_Box\');break;case 2:var div = document.querySelector(\'#SearchInOtherSite\');break;};(div.style.display == \'none\') ? div.style.display = \'inline\' : div.style.display = \'none\';"><img src="https://cdn0.iconfinder.com/data/icons/basic-ui-elements-round/700/09_search-32.png" /></a><div id="SearchInOtherSite" style="display:none">';
...
EH_QuickSearch_Box.innerHTML = '<div id="EH_QuickSearch_Exchange"><div style="font-size:larger;">跳转</div></div><div id="EH_QuickSearch_Popular"><div style="font-size:larger;">常用</div></div><div id="EH_QuickSearch_Used"><div style="font-size:larger;">已使用</div></div><br /><br /><br /><div id="EH_QuickSearch_Plus"><div style="font-size:larger;">要选择的</div></div><br /><br /><br /><div id="EH_QuickSearch_Delete"><div style="font-size:larger;">要排除的</div></div><br /><br /><button id="EH_QuickSearch_Apply">应用</button><button id="EH_QuickSearch_Cancel">取消</button>';
...
document.querySelector('#EH_QuickSearch_Apply').onclick = function () {
  ...
  for (var i = 0; i < input_check.length; i++) {
    Keyword_New += input_check[i].name.replace(/\'\'/g, '"') + ' ';
...
keyword_0 = DeleteSpaceInQuotation(keyword_0).replace(/\s+/g,' ').replace(/^ | $/, '');
...
function EH_QuickSearch_Add2Used(name, name_cn) {
  ...
  div.className = 'EH_QuickSearch_Keyword_Used';
  //div.innerHTML = name_cn + '[' + name + ']';
  div.innerHTML = '<input type="checkbox" checked="true" name="' + name.replace(/"/g, '\'\'') + '" />' + name_cn;
...
function EH_QuickSearch_Add(type, name, name_cn) {
  ...
  switch (type) {
    case '+':
      div.className = 'EH_QuickSearch_Keyword_Plus';
      //div.innerHTML = name_cn + '[' + name + ']';
      div.innerHTML = '<input type="checkbox" name="' + name.replace(/"/g, '\'\'') + '" />' + name_cn;
      document.querySelector('#EH_QuickSearch_Plus').appendChild(div);
      break;
    case '-':
      div.className = 'EH_QuickSearch_Keyword_Delete';
      //div.innerHTML = name_cn + '[' + name + ']';
      div.innerHTML = '<input type="checkbox" name="' + name.replace(/"/g, '\'\'') + '" />' + name_cn;
      document.querySelector('#EH_QuickSearch_Delete').appendChild(div);
      break;
  }
}
