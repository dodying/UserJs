...
;(async function () {
  ...
  let url = `https://2tool.top/kuaizhao.php?k=${encodeURIComponent(window.location.href)}`

  if (blacklist.includes(host)) {
    GM_registerMenuCommand('Cached: Effect ' + host, function () {
      let blacklist = GM_getValue('blacklist', [])
      if (blacklist.includes(host)) {
  ...
  GM_registerMenuCommand('Cached: DO NOT Effect ' + host, function () {
    let blacklist = GM_getValue('blacklist', [])
    if (!blacklist.includes(host)) {
      blacklist.push(host)
      GM_setValue('blacklist', blacklist)
  ...
  if (checkedlist.includes(host)) {
    GM_registerMenuCommand('Cached: Show cached', function () {
      GM_openInTab(url, true)
    })
    return
  }

  let body = await xhrSync(url)
  let args = body.response.match(/doLoadKz\(('.*)\)/g).map(i => i.match(/doLoadKz\(('.*)\)/)[1]).map(i => i.split(',').map(j => j.replace(/^['"](.*)['"]$/, '$1')))
  await Promise.all(args.map(arg => doLoadKz(...arg))).then(result => {
    let caches = result.map(i => i.response.match(/^callback\d\((.*)\)$/)[1]).map(i => JSON.parse(i)).map(i => i.kzUrl).filter(i => i)
    if (caches.length) {
      let checkedlist = GM_getValue('checkedlist', [])
...
async function doLoadKz (s, typeId, num) {
  ...
  var url = preUrl + '/kz.php?s=' + s + '&num=' + num
  return xhrSync(url)
}
