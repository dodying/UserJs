  ...
  if (gE('#riddlecounter')) { //需要答题
    ...
    if (gE('div[style="margin:5px 0 0; color:#FA9300"]')) { //装备损坏
      if (g('option').damageAlert) {
        setAlert('Error', 3);
        _alert(0, new Array('装备损坏，请修复', '裝備損壞，請修復', 'Damaged equipments, please repair'));
      }
      if (g('option').damageFix && location.hash !== '#damageFix') {
        post('?s=Forge&ss=re', 'repair_all=1', function () {
          goto('damageFix', true);
        });
      }
      return;
    }
    if (g('option').autoArena && parseInt(gE('.fd4>div').innerHTML.match(/\d+/) [0]) > g('option').staminaNow) {
      setTimeout(function () {
        autoArena();
...
function main() { //主程序
  if (getValue('disabled')) { //如果禁用
    document.title = _alert( - 1, new Array('hvAutoAttack暂停中', 'hvAutoAttack暫停中', 'hvAutoAttack Paused'));
    gE('.clb>button').innerHTML = '<l0>继续</l0><l1>繼續</l1><l2>Continue</l2>';
    return;
  }
  g('end', false);
  if (parseInt(gE('.fd4>div').innerHTML.match(/\d+/) [0]) <= g('option').staminaNow) {
    setAlert('Error', 3);
    if (!_alert(1, new Array('当前Stamina过低\n或Stamina损失过多\n是否继续？', '當前Stamina過低\n或Stamina損失過多\n是否繼續？', 'Continue?\nYou either have too little Stamina or have lost too much'))) {
      pauseChange();
      return;
    }
  }
  ...
  if (getValue('monsterStatus') && getValue('monsterStatus', true).length === g('monsterAll')) {
    ...
    document.title = _alert( - 1, new Array('monsterStatus错误，正在尝试修复', 'monsterStatus錯誤，正在嘗試修復', 'monsterStatus Error, trying to fix'));
    fixMonsterStatus();
  }
  var bar = gE('.cwb2', 'all');
...
function addStyle(lang) {
  ...
  '#hvAABox{left:calc(50% - 350px);top:50px;font-size:12pt!important;z-index:9999;width:700px;height:510px;display:none;position:absolute;text-align:justify;background-color:#E3E0D1;border:1px solid #000;border-radius:10px;}' +
  ...
  '.hvAATab input[type=number]{width:30px;-moz-appearance:textfield;-webkit-appearance:textfield;text-align:right;}' +
...
function optionBox() { //配置界面
  ...
  '<div id="hvAATab-Skill" class="hvAATab"><l0>注：本标签所有输入框皆表示阈值</l0><l1>注：本標籤所有輸入框皆表示閾值</l1><l2>Note: All input fields in this label indicate thresholds</l2><br><l0>1. OC值 2. 敌人存活数 3. Boss存活数</l0><l1>1. OC值 2. 怪獸存活數 3. Boss存活數</l1><l2>1. OC value 2. Enemies survival number 3. Boss survival number</l2><br><input id="skill" type="checkbox"><label for="skill"><span class="hvAATitle"><l01>其他技能</l01><l2>Other Spells</l2></span> <l0>（按【施放顺序】排序）</l0><l1>（按【施放順序】排序）</l1><l2>(Sort by cast order)</l2></label><br><input id="skill_OFC" type="checkbox"><label for="skill_OFC"><l0>友情小马炮</l0><l1>友情小馬炮</l1><l2>OFC</l2>: </label><input name="skillOC_OFC" type="number"><input name="skillMonster_OFC" type="number"><input name="skillBoss_OFC" type="number"><br><input id="skill_FUS" type="checkbox"><label for="skill_FUS"><l0>龙吼</l0><l1>龍吼</l1><l2>FUS</l2>: </label><input name="skillOC_FUS" type="number"><input name="skillMonster_FUS" type="number"><input name="skillBoss_FUS" type="number"><br><l0>战斗风格</l0><l1>戰鬥風格</l1><l2>Fighting style</l2>: <select name="fightingStyle"><option value="1">二天一流 / Niten Ichiryu</option><option value="2">单手 / One-Handed</option><option value="3">双手 / 2-Handed Weapon</option><option value="4">双持 / Dual Wielding</option><option value="5">法杖 / Staff</option></select><br><input id="skill_3" type="checkbox"><label for="skill_3"><l0>3阶（如果有）</l0><l1>3階（如果有）</l1><l2>T3(if exist)</l2>: </label><input name="skillOC_3" type="number"><input name="skillMonster_3" type="number"><input name="skillOCBoss_3" type="number"><br><input id="skill_2" type="checkbox"><label for="skill_2"><l0>2阶（如果有）</l0><l1>2階（如果有）</l1><l2>T2(if exist)</l2>: </label><input name="skillOC_2" type="number"><input name="skillMonster_2" type="number"><input name="skillOCBoss_2" type="number"><br><input id="skill_1" type="checkbox"><label for="skill_1"><l0>1阶</l0><l1>1階</l1><l2>T1</l2>: </label><input name="skillOC_1" type="number"><input name="skillMonster_1" type="number"><input name="skillOCBoss_1" type="number"></div>' +
  '<div id="hvAATab-Scroll" class="hvAATab"><input id="scroll" type="checkbox"><label for="scroll"><span class="hvAATitle"><l0>使用卷轴</l0><l1>使用捲軸</l1><l2>Use Scrolls</l2></span></label><br><l0>战役模式</l0><l1>戰役模式</l1><l2>Battle type</l2>: <input id="scrollRoundType_ar" type="checkbox"><label for="scrollRoundType_ar">The Arena</label><input id="scrollRoundType_rb" type="checkbox"><label for="scrollRoundType_rb">Ring of Blood</label><input id="scrollRoundType_gr" type="checkbox"><label for="scrollRoundType_gr">GrindFest</label><input id="scrollRoundType_iw" type="checkbox"><label for="scrollRoundType_iw">Item World</label><input id="scrollRoundType_ba" type="checkbox"><label for="scrollRoundType_ba">Random Encounter</label><br><l0>总体条件：当前回合数</l0><l1>總體條件：當前回合數</l1><l2>Conditions: Number of turns</l2> ≥ <input name="scrollRoundNow" type="number"><br><l0>以下输入框为使用相应卷轴的最小回合数</l0><l1>以下輸入框為使用相應捲軸的最小回合數</l1><l2>The following input fields indicate the min round to use the scroll</l2>.<br><input id="scrollFirst" type="checkbox"><label for="scrollFirst"><l0>存在技能生成的Buff时，仍然使用卷轴</l0><l1>存在技能生成的Buff時，仍然使用捲軸</l1><l2>Use Scrolls even when there are effects from spells</l2>.</label><br><input id="scroll_Go" type="checkbox"><label for="scroll_Go">Scroll of the Gods <input name="scrollRound_Go" type="number"></label><br><input id="scroll_Av" type="checkbox"><label for="scroll_Av">Scroll of the Avatar <input name="scrollRound_Av" type="number"></label><br><input id="scroll_Pr" type="checkbox"><label for="scroll_Pr">Scroll of Protection <input name="scrollRound_Pr" type="number"></label><br><input id="scroll_Sw" type="checkbox"><label for="scroll_Sw">Scroll of Swiftness <input name="scrollRound_Sw" type="number"></label><br><input id="scroll_Li" type="checkbox"><label for="scroll_Li">Scroll of Life <input name="scrollRound_Li" type="number"></label><br><input id="scroll_Sh" type="checkbox"><label for="scroll_Sh">Scroll of Shadows <input name="scrollRound_Sh" type="number"></label><br><input id="scroll_Ab" type="checkbox"><label for="scroll_Ab">Scroll of Absorption <input name="scrollRound_Ab" type="number"></label></div>' +
  '<div id="hvAATab-Infusion" class="hvAATab"><input id="infusion" type="checkbox"><label for="infusion"><span class="hvAATitle"><l0>使用魔药</l0><l1>使用魔藥</l1><l2>Use Infusion</l2></span><br><l0>魔药属性与</l0><l1>魔藥屬性與</l1><l2>The style of infusion is the same as Attack Mode in </l2><a href="#hvAATab-Main"><l0>主要选项</l0><l1>主要選項</l1><l2>Main</l2></a><l0>里的攻击模式相同</l0><l1>裡的攻擊模式相同</l1><l2></l2></label><br><l0>战役模式</l0><l1>戰役模式</l1><l2>Battle type</l2>: <input id="infusionRoundType_ar" type="checkbox"><label for="infusionRoundType_ar">The Arena</label><input id="infusionRoundType_rb" type="checkbox"><label for="infusionRoundType_rb">Ring of Blood</label><input id="infusionRoundType_gr" type="checkbox"><label for="infusionRoundType_gr">GrindFest</label><input id="infusionRoundType_iw" type="checkbox"><label for="infusionRoundType_iw">Item World</label><input id="infusionRoundType_ba" type="checkbox"><label for="infusionRoundType_ba">Random Encounter</label><br><l01>使用条件：当前回合数</l01><l2>Conditions: Number of turns</l2> ≥ <input name="infusionRoundNow" type="number"></div>' +
  '<div id="hvAATab-Alarm" class="hvAATab"><span class="hvAATitle"><l0>自定义警报</l0><l1>自定義警報</l1><l2>Alarm</l2></span><br><l0>注意：留空则使用默认音频，建议每个用户使用自定义音频</l0><l1>注意：留空則使用默認音頻，建議每個用戶使用自定義音頻</l1><l2>Note: Leave the box blank to use default audio, it\'s recommended for all user to use custom audio.</l2><br><l01>通用</l01><l2>Common</l2>: <input name="audio-default" type="text"><br><l0>错误</l0><l1>錯誤</l1><l2>Error</l2>: <input name="audio-Error" type="text"><br><l0>失败</l0><l1>失敗</l1><l2>Defeat</l2>: <input name="audio-Failed" type="text"><br><l0>答题</l0><l1>答題</l1><l2>Riddle</l2>: <input name="audio-Riddle" type="text"><br><l0>胜利</l0><l1>勝利</l1><l2>Victory</l2>: <input name="audio-Win" type="text"><div class="testAlarms"></div></div>' +
  '<div id="hvAATab-Rule" class="hvAATab"><span class="hvAATitle"><l0>攻击规则</l0><l1>攻擊規則</l1><l2>Attack Rule</l2></span> <a href="https://github.com/dodying/UserJs/blob/master/HentaiVerse/hvAutoAttack/README.md#attack_rule" target="_blank"><l01>示例</l01><l2>Example</l2></a><div>1. <l0>每回合计算敌人当前血量，血量最低的设置初始血量为10，其他敌人为当前血量倍数*10</l0><l1>每回合計算敌人當前血量，血量最低的設置初始血量為10，其他敌人為當前血量倍數*10</l1><l2>Each enemiy is assigned a number which is used to determine the target to attack, let\'s call that number Priority Weight or PW.</l2></div><div>2. <l0>初始权重与下述各Buff权重相加</l0><l1>初始權重與下述各Buff權重相加</l1><l2>PW(X) = 10 * Max_HP(X) / Min(Max_HP(All_enemies)) + Accumulated_Weight_of_Deprecating_Spells_In_Effect(X)</l2><br>Sleep: <input name="weight_Sle" placeholder="5" step="0.1" type="number"> Blind: <input name="weight_Bl" placeholder="3" step="0.1" type="number"> Slow: <input name="weight_Slo" placeholder="3" step="0.1" type="number"> Imperil: <input name="weight_Im" placeholder="-5" step="0.1" type="number"><br>Coalesced Mana: <input name="weight_CM" placeholder="-5" step="0.1" type="number"> MagNet: <input name="weight_MN" placeholder="-4" step="0.1" type="number"> Silence: <input name="weight_Si" placeholder="-4" step="0.1" type="number"> Drain: <input name="weight_Dr" placeholder="-4" step="0.1" type="number"><br>Weaken: <input name="weight_We" placeholder="-4" step="0.1" type="number"> Confuse: <input name="weight_Co" placeholder="-1" step="0.1" type="number">Stunned: <input name="weight_Stun" placeholder="-4" step="0.1" type="number"></div><div><div class="hvAANew"></div>3. <l0>如果敌人相邻两只敌人已死（或不存在），则给其权重增加</l0><l1>如果敵人相鄰兩隻敵人已死（或不存在），則給其權重增加</l1><l2>If the enemy of which adjacent two enemies dead (or does not exist), then its PW increased</l2> <input name="weight_Wall" placeholder="0.5" step="0.1" type="number"></div><div>4. <l0>计算出最终权重，攻击权重最小的敌人</l0><l1>計算出最終權重，攻擊權重最小的敌人</l1><l2>Whichever enemy has the lowest PW will be the target.</l2></div><div>5. <l0>如果你对各Buff权重有特别见解，请务必</l0><l1>如果你對各Buff權重有特別見解，請務必</l1><l2>If you have any suggestions, please </l2><a href="#hvAATab-About"><l0>告诉我</l0><l1>告訴我</l1><l2>let me know</l2></a>.</div></div>' +
  '<div id="hvAATab-Drop" class="hvAATab hvAACenter"><span class="hvAATitle"><l01>掉落监测</l01><l2>Drops Tracking</l2></span><button class="reMonitor"><l01>重置</l01><l2>Reset</l2></button></div>' +
  '<div id="hvAATab-About" class="hvAATab hvAACenter"><div><span class="hvAAFeedback"><l0>反馈</l0><l1>反饋</l1><l2>Feedback</l2>: <a href="https://github.com/dodying/UserJs/issues/" target="_blank">1. GitHub</a><a href="https://greasyfork.org/scripts/18482/feedback" target="_blank">2. GreasyFork</a><a href="http://e-hentai.org/dmspublic/karma.php?u=2565471" target="_blank">3. +K</a><a href="https://gitter.im/dodying/UserJs" target="_blank">4. Gitter</a></span></div><div><span class="hvAATitle"><l0>当前状况</l0><l1>當前狀況</l1><l2>Current status</l2>: </span><br><l0>如果脚本长期暂停且网络无问题，请点击【临时修复】</l0><l1>如果腳本長期暫停且網絡無問題，請點擊【臨時修復】</l1><l2>If the script does not work and you are sure that it\'s not because of your internet, click [Try to fix]</l2><br><l0>战役模式</l0><l1>戰役模式</l1><l2>Battle type</l2>: <select class="hvAADebug" name="roundType"><option></option><option value="ar">The Arena</option><option value="rb">Ring of Blood</option><option value="gr">GrindFest</option><option value="iw">Item World</option><option value="ba">Random Encounter</option></select><br><l0>当前回合</l0><l1>當前回合</l1><l2>Current round</l2>: <input name="roundNow" class="hvAADebug" placeholder="1" type="number"> <l0>总回合</l0><l1>總回合</l1><l2>Total rounds</l2>: <input name="roundAll" class="hvAADebug" placeholder="1" type="number"><br><button class="hvAAFix"><l0>尝试修复</l0><l1>嘗試修復</l1><l2>Try to fix</l2></button></div><div class="hvAAQuickSite"><span class="hvAATitle"><div class="hvAANew"></div><l0>快捷站点</l0><l1>快捷站點</l1><l2>Quick Site</l2></span><br><l0>留空“姓名”输入框则会表示删除，修改完成后请及时保存</l0><l1>留空“姓名”輸入框則會表示刪除，修改完成後請及時保存</l1><l2> The input box left "name" blank will be deleted, after change please save in time.</l2></div><div><button class="hvAAExport"><l0>导出设置</l0><l1>導出設置</l1><l2>Export Confiuration</l2></button><button class="hvAAImport"><l0>导入设置</l0><l1>導入設置</l1><l2>Import Confiuration</l2></button><textarea class="hvAAConfig"></textarea></div></div>' +
  '<div id="hvAATab-Recommend" class="hvAATab hvAACenter"><span class="hvAATitle"><l0>推荐脚本</l0><l1>推薦腳本</l1><l2>Recommend</l2></span><table><tbody><tr><td><l0>名称</l0><l1>名稱</l1><l2>Name</l2></td><td><l0>应用到</l0><l1>應用到</l1><l2>Applies to</l2></td><td><l0>说明</l0><l1>說明</l1><l2>Description</l2></td><td>URL</td></tr><tr><td><l0>【HV】购物清单</l0><l1>【HV】購物清單</l1><l2>hvBazaarList</l2></td><td><a href="?s=Bazaar&ss=is" target="_blank">Bazaar/Item Shop</a></td><td><l0>快速生成购物清单</l0><l1>快速生成購物清單</l1><l2>Quickly generate shopping lists</l2></td><td><a href="https://github.com/dodying/UserJs/raw/master/HentaiVerse/hvBazaarList.user.js" target="_blank">Install</a></td></tr><tr><td><l0>【HV】喂食</l0><l1>【HV】餵食</l1><l2>hvFeed</l2></td><td><a href="?s=Bazaar&ss=ml" target="_blank">Monster Lab</a></td><td><l0>自动给大于100级的怪物喂开心药丸</l0><l1>自動給大於100級的怪物餵開心藥丸</l1><l2>Automatically feed monster of which level more than 100 on happy pills</l2></td><td><a href="https://github.com/dodying/UserJs/raw/master/HentaiVerse/hvFeed.user.js" target="_blank">Install</a></td></tr><tr><td>Reloader</td><td></td><td><l01>已内置</l01><l2>Built-in</l2></td><td><a href="https://forums.e-hentai.org/index.php?showtopic=65126&st=2660&p=4384894&#entry4384894" target="_blank">E-Hentai Forums</a></td></tr><tr><td>RiddleLimiter Plus</td><td></td><td><l01>已内置</l01><l2>Built-in</l2></td><td><a href="https://forums.e-hentai.org/index.php?showtopic=65126&st=1020&p=3000982&#entry3000982" target="_blank">E-Hentai Forums</a></td></tr><tr><td>HV Random Encounter Notification</td><td></td><td></td><td><a href="http://forums.e-hentai.org/index.php?showtopic=65126&st=1000&p=2990345&#entry2990345" target="_blank">E-Hentai Forums</a></td></tr><tr><td>HV Equipment Comparison</td><td></td><td></td><td><a href="http://forums.e-hentai.org/index.php?s=&showtopic=65126&view=findpost&p=4492842" target="_blank">E-Hentai Forums</a></td></tr><tr><td>HentaiVerse Better Equipment Shop</td><td></td><td></td><td><a href="https://forums.e-hentai.org/index.php?showtopic=65126&st=800&p=2750319&#entry2750319" target="_blank">E-Hentai Forums</a></td></tr></tboby></table></div>' +
  '<div id="hvAATab-ChangeLog" class="hvAATab"><span class="hvAATitle"><l0>更新日志</l0><l1>更新日誌</l1><l2>Change log</l2></span><div>v2.66<br>1. <l0>选项去除: 相当于默认勾选，如Reloader</l0><l1>選項去除: 相當於默認勾選，如Reloader</l1><l2>Options Removed: equal to been checked, such as Reloader</l2><br>2. <l0>选项变更: 装备损坏、任意页面停留竞技场</l0><l1>選項變更: 裝備損壞、任意頁面停留競技場</l1><l2>Options Changed: such as "Damaged Equippment", "Idle Arena"</l2><br>3. <l0>选择新增: 小马弹警框、所有敌人Imperil</l0><l1>選擇新增: 小馬彈警框、所有敵人Imperil</l1><l2>Options Added: "Riddle Alert", "All Enemies Imperil"</l2><br>4. <l0>自动修复: 当敌人状况缺失或数量不对，自行修复</l0><l1>自動修復: 當敵人狀況缺失或數量不對，自行修復</l1><l2>Auto Fix: If some ERROR happened with Enemy Status, auto fix it</l2><br>5. <l0>快捷链接: 可在"关于本脚本"中进行相关设置</l0><l1>快捷鏈接: 可在"關於本腳本"中進行相關設置</l1><l2>Quick Site: can config in tab "About this"</l2><br>6. <l0>攻击规则: 如果敌人相邻两只敌人已死（或不存在），则给其权重增加0.5（相当于不优先攻击这些敌人）</l0><l1>攻擊規則: 如果敵人相鄰兩隻敵人已死（或不存在），則給其權重增加0.5（相當於不優先攻擊這些敵人）</l1><l2>Attack Rule: If the enemy of which adjacent two enemies dead (or does not exist), then its PW increased 0.5 (equal to dont attack these enemies later)</l2></div></div>' +
  ...
  gE('.hvAAShowAbout', optionBox).onclick = function () {
    ...
    if (getValue('quickSite')) {
      ...
      for (i = 0; i < quickSite.length; i++) {
        _html += '<tr><td><input type="text" value="' + quickSite[i].fav + '"></td><td><input type="text" value="' + quickSite[i].name + '"></td><td><input type="text" value="' + quickSite[i].url + '"></td></tr>';
      }
      _html += '</tbody>';
      var table = cE('table');
      table.innerHTML = _html;
  ...
  gE('.testAlert', optionBox).onclick = function () {
    ...
    setTimeout(function () {
      _alert(0, new Array('请勾选“允许来自 hentaiverse.org 的对话框将您带往标签页”', '请勾选“允许来自 hentaiverse.org 的对话框带您前往分页”', 'Please check "allow dialogs from hentaiverse.org to take you to their tab"'));
    }, 3000);
  }
  ...
  var levelsInput = gE('.hvAAArenaLevels input[type=checkbox]', 'all', optionBox);
  var i;
  var j;
  for (i = 0; i < levelsInput.length; i++) {
    levelsInput[i].onclick = function () {
      var input = gE('.hvAAArenaLevels input[type=checkbox]', 'all');
      var out = new Array();
      for (j = 0; j < input.length; j++) {
        if (input[j].checked) out.push(input[j].name);
      }
  ...
  gE('.hvAAApply', optionBox).onclick = function () {
    ...
    if (gE('select[name=attackStatus] option[value="-1"]:checked', optionBox)) {
      _alert(0, new Array('请选择攻击模式', '請選擇攻擊模式', 'Please select the attack mode'));
    ...
    for (var i = 0; i < inputs.length; i++) {
      ...
      if (inputs[i].type === 'number') {
      ...
      } else if (inputs[i].type === 'text' && !inputs[i].outerHTML.match(/check="(.*?)"/)) {
        ...
        if (new RegExp(inputs[i].outerHTML.match(/check="(.*?)"/) [1]).test(inputs[i].value) || inputs[i].value === '') {
          ...
          _alert(0, new Array('请检查高亮的输入框是否填写正确', '請檢查高亮的輸入框是否填寫正確', 'Please check if the highlighted input boxes are filled in correctly'));
...
function autoArena() { //自动刷竞技场
  ...
  post(location.href, 'recover=all', function () { //回复
    document.title = _alert( - 1, new Array('恢复完成', '恢復完成', 'Recover success'));
    post('?s=Battle&ss=ar', 'arenaid=' + arena.array[arena.array.length - 1], function () {
      document.title = _alert( - 1, new Array('竞技场开始', '競技場開始', 'Arena start'));
...
function continueBattle() { //自动前进
  if (g('monsterAlive') === 0 && g('option').dropMonitor) {
    ...
    drop['#0_Turn'] = ('#0_Turn' in drop) ? drop['#0_Turn'] + parseInt(gE('#togpane_log>table>tbody>tr>td').innerText)  : 1;
...
function battleStart() { //New Round
  ...
  g('roundType', (function () {
    if (getValue('roundType') && getValue('roundType') !== '') {
      ...
      var roundType;
      var temp = battleLog[battleLog.length - 2].innerHTML;
  ...
  if (/You lose \d+ Stamina/.test(battleLog[0].innerText)) {
...
function autoUseDeSkill() { //自动施法De技能
  ...
  for (var i in skillLib) {
    if (g('option') ['debuffSkill_' + i] && isOn(skillLib[i].id) && !gE('#mkey_' + g('monsterStatus') [0].id + '>.btm6>img[src*="' + skillLib[i].img + '"]') && gE('#mkey_' + g('monsterStatus') [0].id + '>.btm6>img', 'all').length < 6) {
      gE(skillLib[i].id).click();
...
function goto(url, reload) {
  ...
  if (typeof url === 'undefined' && typeof reload === 'undefined') {
    ...
    history.pushState(null, null, raw + '#' + url);
...
function _alert(func, langs) {
  var lang = langs[gE('select[name="lang"]').value];
  if (func === - 1) {
    return lang;
  } else if (func === 0) {
    alert(lang);
...
function allImperiled() { //给所有敌人施放Imperil
  ...
  for (var i = - 2; ; ) {
    ...
    if (!gE('img[src*="imperil"]', monsterBuff[i]) && isOn('213') && !g('monsterStatus') [i].isDead && gE('img', 'all', monsterBuff[i]).length < 6) {
      gE('213').click();
