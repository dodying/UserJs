...
(function init() {
  ...
  if (gE('#riddlecounter')) { //需要答题
    ...
    } else if (parseInt(gE('.fd4>div').textContent.match(/\d+/) [0]) > g('option').staminaNow) {
...
function optionBox() { //配置界面
  ...
  optionBox.innerHTML = [
    ...
    '<div class="hvAATab" id="hvAATab-ChangeLog"><span class="hvAATitle"><l0>更新日志</l0><l1>更新日誌</l1><l2>Change log</l2> <a href="https://github.com/dodying/UserJs/blob/master/HentaiVerse/hvAutoAttack/#更新历史" target="_blank"><l01>更多</l01><l2>more</l2></a></span><div>v2.73 <br>1. <l0>更多自定义判断条件</l0><l1>更多自定義判斷條件</l1><l2>More customize conditions</l2>, <a href="https://github.com/dodying/UserJs/blob/master/HentaiVerse/hvAutoAttack/#自定义判断条件" target="_blank"><l0>具体用法</l0><l1>具體用法</l1><l2>Specific usage</l2></a><br>2. <l0>内置Random Encounter Notification(并不，其实是自己写的简陋的版本)</l0><l1>內置Random Encounter Notification(並不，其實是自己寫的簡陋的版本)</l1><l2>New Built-in Plugin: Random Encounter Notification</l2><br>3. <l0>其他一些变更</l0><l1>其他一些變更</l1><l2>Some other changes</l2><br><span style="color:red;font-weight:bold;">PS. <l0>由于自定义判断条件，所以标签页“技能”、“卷轴”、“魔药”的使用条件都需要重新设置</l0><l1>由於自定義判斷條件，所以標籤頁“技能”、“捲軸”、“魔藥”的使用條件都需要重新設置</l1><l2>Because of customize conditions, the conditions in the tab "skill", "scroll", "infusion" need to config.</l2></span></div></div>',
    '</div>',
...
function riddleAlert() { //答题警报
  if (g('option').riddleAlert && location.hash !== '#riddleAlert' && /Firefox/g.test(navigator.userAgent)) {
...
function randomEncounterCheck() { //Random Encounter Notification
  var dateNow = new Date();
  dateNow = dateNow.getUTCFullYear() + '/' + (dateNow.getUTCMonth() + 1) + '/' + dateNow.getUTCDate();
