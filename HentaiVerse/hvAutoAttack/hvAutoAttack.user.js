...
(function init() {
  ...
  if (gE('#riddlecounter')) { //需要答题
    ...
    if (/Android|iPhone|iPad/gi.test(navigator.userAgent)) { //移动端
      setAlert('Riddle');
      function autoPlay() {
        gE('.hvAAAlert').load();
...
function optionBox() {
  ...
  '<div class="hvAATab" id="hvAATab-Alarm"><span class="hvAATitle"><l0>自定义警报</l0><l1>自定義警報</l1><l2>Alarm</l2></span><br><l0>注意：留空则使用默认音频，建议每个用户使用自定义音频</l0><l1>注意：留空則使用默認音頻，建議每個用戶使用自定義音頻</l1><l2>Note: Leave the box blank to use default audio, it\'s recommended for all user to use custom audio.</l2><div><l01>通用</l01><l2>Common</l2>: <input name="audio-Common" type="text"><br><l0>错误</l0><l1>錯誤</l1><l2>Error</l2>: <input name="audio-Error" type="text"><br><l0>失败</l0><l1>失敗</l1><l2>Defeat</l2>: <input name="audio-Defeat" type="text"><br><l0>答题</l0><l1>答題</l1><l2>Riddle</l2>: <input name="audio-Riddle" type="text"><br><l0>胜利</l0><l1>勝利</l1><l2>Victory</l2>: <input name="audio-Victory" type="text"></div><div><l0>请将将要测试的音频文件的地址填入这里</l0><l1>請將將要測試的音頻文件的地址填入這裡</l1><l2>Plz put in the audio file address you want to test</l2>:<br><input class="hvAADebug" name="audio-Text" type="text"></div></div>' +
  ...
  '<div class="hvAATab" id="hvAATab-ChangeLog"><span class="hvAATitle"><l0>更新日志</l0><l1>更新日誌</l1><l2>Change log</l2></span><div>v2.69<br>1. <l0>功能回归: 掉落监测(WoC，因为函数Reloader变更，原先的独立脚本失效了...)</l0><l1>功能回歸: 掉落監測(吔屎啦，因為函數Reloader變更，原先的獨立腳本失效了...)</l1><l2>Feature Regression: "dropMonitor" (WTF, because the function Reloader changed, the script I separated before invalid.)</l2><br>2. <l0>移动端优化</l0><l1>移動端優化</l1><l2>Phone optimization</l2></div></div>' +
...
function battleInfo() { //战斗战况
  ...
  gE('.hvAALog').innerHTML = 'Turns: ' + g('runTime') + '<br>Speed: ' + g('runSpeed') + ' turns/s<br>Round: ' + g('roundNow') + '/' + g('roundAll') + '<br><l0>攻击模式</l0><l1>攻擊模式</l1><l2>Attack Mode</l2>: ' + status[g('attackStatus')] + '<br><l0>存活Boss</l0><l1>存活Boss</l1><l2>Bosses</l2>: ' + g('bossAlive') + '<br><l0>敌人</l0><l1>敌人</l1><l2>Monsters</l2>: ' + g('monsterAlive') + '/' + g('monsterAll');
