...
function main() { //主程序
  ...
  if (g('end')) return;
  if (g('option').debuffSkill && g('option').debuffSkillAllIm && gE('div.btm6 img[src*="imperil"]', 'all').length < g('monsterAlive')) allImperiled(); //给所有敌人上Imperil
  if (g('end')) return;
...
function post(href, parm, func) { //post
  ...
  xhr.timeout = g('option').delayTimeoutTime * 1000;
...
function setAlarm(e) { //发出警报
  ...
  if (g('option').alert && g('option') ['audioEnable_'] + e) setAudioAlarm(e);
}
function setAudioAlarm(e) { //发出音频警报
  ...
  audio.src = (g('option') ['audio_' + e]) ? g('option') ['audio_' + e] : 'https://raw.githubusercontent.com/dodying/UserJs/master/HentaiVerse/hvAutoAttack/' + e + fileType;
  audio.controls = true;
  audio.loop = (e === 'Riddle') ? true : false;
  audio.play();
  gE('body').appendChild(audio);
  unsafeWindow.addEventListener('mousemove', function pauseAudio() {
    audio.pause();
    unsafeWindow.removeEventListener('touchstart', pauseAudio, false);
  }, false);
}
