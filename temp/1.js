...
function autoUseBuffSkill() { //自动使用药水、施法增益技能
  ...
  if (gE('div.bte>img[src*="channeling"]')) {
    ...
    for (i = 0; i < skillPack.length; i++) {
      j = skillPack[i];
      if (g('option') ['channelSkill_' + j] && !gE('div.bte>img[src*="' + skillLib[j].img + '"]') && isOn(skillLib[j].id)) {
    ...
    for (i = 0; i < skillPack.length; i++) {
      j = skillPack[i];
      if (g('option') ['buffSkill_' + j] && !gE('div.bte>img[src*="' + skillLib[j].img + '"]') && isOn(skillLib[j].id)) {
        gE(skillLib[j].id).click();
        g('end', true);
        return;
      }
...
function autoUseDeSkill() { //自动施法De技能
  ...
  for (i = 0; i < skillPack.length; i++) {
    j = skillPack[i];
    if (g('option') ['debuffSkill_' + j] && isOn(skillLib[j].id) && !gE('img[src*="' + skillLib[j].img + '"]', monsterBuff)) {
