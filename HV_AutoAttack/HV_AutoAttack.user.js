...
function main() { //主程序
  ...
  if (parseInt(ge(0, '.fd4>div').innerHTML.replace('Stamina: ', '')) < 10) {
    ge(2, '1001').click;
    return;
  }
  runtime++;
...
function CountMonsterHP() { //统计怪物血量
  ...
  for (var i = 0; i < Monter_HP.length; i++) {
    if (Monter_HP[i].querySelector('img[src="/y/s/nbardead.png"]')) {
      ...
      if (Monter_HP[i].querySelector('.monsterHp')) {
        var hpDiv = Monter_HP[i].querySelector('.monsterHp');
