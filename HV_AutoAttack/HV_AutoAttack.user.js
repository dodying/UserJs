...
function main() { //主程序
  if (parseInt(document.querySelector('.fd4>div').innerHTML.replace('Stamina: ', '')) < 10) {
    document.getElementById('1001').click;
    return;
  }
  runtime++;
  window.Monster_Count_All = document.querySelectorAll('div.btm1').length;
  window.Monster_Count_Dead = document.querySelectorAll('img[src*="nbardead"]').length;
  ...
  if (localStorage.HVAA_Monster_Status) {
    ...
    alert('请点击临时修复');
    document.querySelector('#HV_AutoAttack_Option').style.display = 'block';
    document.querySelector('#HVAA_Bu_Fi').style.zIndex = 1;
    document.querySelector('#HVAA_Fix').focus();
  }
...
function OptionButton() { //配置
  ...
  HV_AutoAttack_Button.innerHTML = '<a href="javascript:#"><img src="https://raw.githubusercontent.com/dodying/UserJs/master/HV_AutoAttack/Setting.png" /></a>';
  ...
  if (document.querySelector('#togpane_log')) {
    ...
    div.innerHTML = '当前总共有' + document.querySelectorAll('div.btm1').length + '只怪物，本地储存' + Monster_Status_length + '只怪物。';
    div.innerHTML += (document.querySelectorAll('div.btm1').length === Monster_Status_length) ? '数目相同' : '<br><span style="color:red;">数目不同，请点击<b>【临时修复】</b></span>';
    HV_AutoAttack_Option.querySelector('#HVAA_Fix_Monster').appendChild(div);
  }
  if (localStorage.HVAA_Monster_Status && document.querySelector('#togpane_log')) {
...
function RiddleAlert() { //答题警报
  if (document.querySelector('#riddlecounter')) {
    document.title = '！！！紧急';
      ...
      for (var i = 0; i < 30; i++) {
        setTimeout(function () {
          ...
          if (parseInt(time) <= parseInt(HVAA_Setting.Riddle_Answer_Time)) {
            if (!document.querySelector('#riddlemaster').value) {
...
function DeadSoon() { //自动回血回魔
  ...
  if (HP <= HVAA_Setting.HP2 * 0.01) { //自动回血
    if (!document.querySelector('.cwb2[src*="barsilver"]')) {
...
function AutoUsePotAndSuSkill() { //自动使用药水、施法增益技能
  ...
  if ((Round_All >= HVAA_Setting.All_Mode_All_Round) || (Round_All === Round_Now && Round_All === 1)) {
    if (document.querySelector('div.bte>img[src*="channeling"]')) {
      ...
      if (buff.length > 0) {
        for (var n = 0; n < buff.length; n++) {
          ...
          if (buff_lasttime <= HVAA_Setting.Ch_ReSkill) {
            if (spell_name === 'Cloak of the Fallen' && !document.querySelector('div.bte>img[src*="sparklife"]') && document.getElementById('422').style.opacity !== '0.5') {
        ...
        for (var i in Skill_Lib) {
          if (HVAA_Setting['Ch_Skill_' + i] !== undefined) {
            if (HVAA_Setting['Ch_Skill_' + i] && !document.querySelector('div.bte>img[src*="' + Skill_Lib[i].img + '"]') && document.getElementById(Skill_Lib[i].id)) {
              if (document.getElementById(Skill_Lib[i].id).style.opacity !== '0.5') {
                document.getElementById(Skill_Lib[i].id).click();
                window.HVAA_End = true;
                return;
    ...
    if (!document.querySelector('div.bte>img[src*="healthpot"]') && HP < 1 && HVAA_Setting.Su_Skill_HD && document.querySelector('.bti3>div[onmouseover*="Health Draught"]')) {
      document.querySelector('.bti3>div[onmouseover*="Health Draught"]').click();
      window.HVAA_End = true;
      return;
    } else if (!document.querySelector('div.bte>img[src*="manapot"]') && MP < 1 && HVAA_Setting.Su_Skill_MD && document.querySelector('.bti3>div[onmouseover*="Mana Draught"]')) {
      document.querySelector('.bti3>div[onmouseover*="Mana Draught"]').click();
      window.HVAA_End = true;
      return;
    } else if (!document.querySelector('div.bte>img[src*="spiritpot"]') && SP < 0.8 && HVAA_Setting.Su_Skill_SD && document.querySelector('.bti3>div[onmouseover*="Spirit Draught"]')) {
    ...
    for (var i in Skill_Lib) {
      if (HVAA_Setting['Su_Skill_' + i] !== undefined) {
        if (HVAA_Setting['Su_Skill_' + i] && !document.querySelector('div.bte>img[src*="' + Skill_Lib[i].img + '"]') && document.getElementById(Skill_Lib[i].id)) {
          if (document.getElementById(Skill_Lib[i].id).style.opacity !== '0.5') {
            document.getElementById(Skill_Lib[i].id).click();
            window.HVAA_End = true;
            return;
    ...
    if (!document.querySelector('div.bte>img[src*="sparklife"]') && document.getElementById('422').style.opacity !== '0.5') {
      document.getElementById('422').click();
      window.HVAA_End = true;
      return;
    }
...
function AutoUseInfusions() {
  ...
  if (document.querySelector('.bti3>div[onmouseover*="' + Infusions_Lib[HVAA_Setting.Infusions_Status].name + '"]') && !document.querySelector('div.bte>img[src*="' + Infusions_Lib[[HVAA_Setting.Infusions_Status]].img + '"]')) {
    document.querySelector('.bti3>div[onmouseover*="' + Infusions_Lib[HVAA_Setting.Infusions_Status].name + '"]').click();
    window.HVAA_End = true;
    return;
  }
} //////////////////////////////////////////////////
  ...
  if (HVAA_Setting.De_Skill && ((window.Monster_Count_Boss_Alive > 0 && HVAA_Setting.De_Skill_Boss && window.Monster_Count_Alive >= HVAA_Setting.De_Skill_Boss_Count) || HVAA_Setting.De_Skill_All)) {
    ...
    for (var i in Skill_Lib) {
      if (HVAA_Setting['De_Skill_Boss_' + i] && window.Monster_Count_Boss_Alive > 0 && HVAA_Setting.De_Skill_Boss && Skill_Lib[i].effect && window.Monster_Count_Alive >= HVAA_Setting.De_Skill_Boss_Count) {
          ...
          if (document.getElementById(Skill_Lib[i].id).style.opacity !== '0.5') {
            for (var j = Monster_Status.length - 1; j >= 0; j--) {
              if (!Monster_Status[j].isDead) {
                var Monster_Find = (!document.querySelector('#mkey_' + Monster_Status[j].id + '>.btm6>img[src*="' + Skill_Lib[i].img + '"]')) ? true : false;
                break;
              }
            }
            if (Monster_Find) {
        ...
        if (document.getElementById(Skill_Lib[i].id) && !document.querySelector('#mkey_' + Monster_Status[0].id + '>.btm6>img[src*="' + Skill_Lib[i].img + '"]')) {
          if (document.getElementById(Skill_Lib[i].id).style.opacity !== '0.5') {
            document.getElementById(Skill_Lib[i].id).click();
...
function AutoAttack() { //自动打怪
  if (window.oc >= HVAA_Setting.Spirit_Stance_OC && HVAA_Setting.Spirit_Stance && !document.querySelector('#ckey_spirit[src*="spirit_a"]')) {
    document.querySelector('#ckey_spirit').click();
  }
  Monster_Status.sort(ArrCom('Fin_Weight'));
  var minnum = Monster_Status[0].id;
  if (Attack_Status !== 0) {
    if (document.getElementById('1' + Attack_Status + '1')) document.getElementById('1' + Attack_Status + '1').click();
    if (Monster_Count_Alive >= HVAA_Setting.Mi_Skill && document.getElementById('1' + Attack_Status + '2')) document.getElementById('1' + Attack_Status + '2').click();
    if (Monster_Count_Alive >= HVAA_Setting.Hi_Skill && document.getElementById('1' + Attack_Status + '3')) document.getElementById('1' + Attack_Status + '3').click();
  }
  if (HVAA_Setting.Sp_Skill) {
    /*
    if (HVAA_Setting.Sp_Skill_FUS && (Monster_Count_Alive > HVAA_Setting.Sp_Skill_FUS_Monster || Monster_Count_Boss_Alive > HVAA_Setting.Sp_Skill_FUS_Boss) && document.getElementById(''))     document.getElementById('').click();
    */
    if (HVAA_Setting.Sp_Skill_OFC && (Monster_Count_Alive > HVAA_Setting.Sp_Skill_OFC_Monster || Monster_Count_Boss_Alive > HVAA_Setting.Sp_Skill_OFC_Boss) && document.getElementById('1111')) document.getElementById('1111').click();
  }
