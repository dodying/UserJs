...
function main() { //主程序
  ...
  if (HVAA_Setting.De_Skill && window.Monster_Count_Boss_Alive > 0) { //自动施法De技能
    AutoUseDeSkill();
  }
  if (window.HVAA_End) {
    window.HVAA_End = false;
...
function AutoUseDeSkill() { //自动施法De技能
  ...
  Monster_Status.sort(ArrCom('isBoss'));
  for (var i in Skill_Lib) {
    if (HVAA_Setting['De_Skill_' + i] && document.getElementById(Skill_Lib[i].id)) {
      if (document.getElementById(Skill_Lib[i].id).style.opacity !== '0.5') {
        document.getElementById(Skill_Lib[i].id).click();
        for (var j = 0; j < Monster_Count_All; j++) {
          if (Monster_Status[j].isBoss && !document.querySelector('#mkey_' + eval(Monster_Status[j].id + 1) + '>.btm6>img[src*="/e/' + Skill_Lib[i].img + '.png"]')) {
            var Boss_find = true;
            break;
          }
        }
      ...
      if (Boss_find) {
        document.querySelector('#mkey_' + eval(Monster_Status[j].id + 1)).click();
        window.HVAA_End = true;
        return;
      }
    }
